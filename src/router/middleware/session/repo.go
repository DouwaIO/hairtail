...
import (
	...
	"gitlab.com/douwa/registry/dougo/src/dougo/model"
	"gitlab.com/douwa/registry/dougo/src/dougo/remote"
...
func SetRepoAndUser() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !ok {
			...
			log.Debugf("vvvvvvvvvvvvvvvvvv %s",
				v,
			)

		var (
			gitname = c.Query("git_name")
		...
		user, err := store.GetUserNameToken(c, project.ID, gitname, oauth2)
		if err != nil {
			log.Debugf("cannot find user %s",
				err.Error(),
			)
			c.String(500, "内部错误")
			c.Abort()
